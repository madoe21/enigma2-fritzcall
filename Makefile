...
PLUGIN_NAME   = fritz_call
PACKAGE_NAME  = enigma2-plugin-extensions-fritz-call
...
copy-settings:
	...
	@if grep -qE '^FRITZ_(HOST|USER|PASSWORD)=' .env 2>/dev/null; then \
		...
		ssh -p $(BOX_PORT) $(BOX_USER)@$(BOX_HOST) 'set -e; \
			...
			touch $$SF; \
			grep -v "^config\.plugins\.fritzcall\." $$SF > $$TMP || true; \
			...
			[ -n "$$HOST" ] && printf "%s\n" "config.plugins.fritzcall.host=$$HOST" >> $$TMP; \
			[ -n "$$USER" ] && printf "%s\n" "config.plugins.fritzcall.username=$$USER" >> $$TMP; \
			[ -n "$$PASS" ] && printf "%s\n" "config.plugins.fritzcall.password=$$PASS" >> $$TMP; \
